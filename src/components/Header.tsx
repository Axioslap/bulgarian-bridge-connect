...
const Header = () => {
  ...
  const NavLink = ({
    ...
    path: string;
    label: string;
  }) => <Link to={path} className={`nav-link px-3 py-2 text-sm font-medium transition-all duration-200 relative group ${isActiveRoute(path) ? 'text-gray-900 font-bold' : 'text-gray-700 hover:text-gray-900'}`}>
      {label}
  ...
  const MobileMenuButton = () => <div className="md:hidden">
      <Button variant="ghost" size="icon" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="text-gray-700 hover:text-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <nav className="hidden md:flex space-x-8">
            {navLinks.map(link => <NavLink key={link.path} path={link.path} label={link.label} />)}
          </nav>
